...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'updateEOA') {
    $stmt = $connection->prepare("UPDATE eoa 
        SET 
            session = ? 
            -- path=? 
        WHERE 
    ...
    if ($stmt->execute()) {
        echo json_encode(['status' => 200, 'message' => 'EOA updated successfully']);
    } else {
        echo json_encode(['status' => 500, 'message' => 'Failed to update EOA']);
    }
}
